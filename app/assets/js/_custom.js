document.addEventListener("DOMContentLoaded", function () {
	...
	let slickArrowWhite = '<svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 41 41"><path d="M15.3 20.5c0 .3.1.6.3.8l6.5 7.8a1.3 1.3 0 1 0 2-1.7l-5.8-6.9 5.6-7a1.3 1.3 0 0 0-.2-1.7 1.3 1.3 0 0 0-2 .1l-6.1 7.8c-.2.2-.3.5-.3.8Z" fill="#fff"/><circle r="20" transform="matrix(0 -1 -1 0 20.5 20.5)" stroke="#fff"/></svg>',
		slickArrowBlack = '<svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 60"><path stroke="#12404F" d="M.5.5h59v59H.5z"/><path d="M31.8 37a1 1 0 0 1-.8-.4l-4.8-6a1 1 0 0 1 0-1.2l5-6a1 1 0 0 1 1.6 1.2L28.3 30l4.3 5.4a1 1 0 0 1-.8 1.6Z" fill="#12404F"/></svg>';
	...
	/**Start Mobile Menu */
	let toggleStopScroll = () => {
		document.body.classList.toggle('stopscroll');
		document.querySelectorAll('.site-header--fixed').forEach(el => {
			el.classList.toggle('site-header--pr');
		})
	}
	let toggleMenu = function () {
		let menuButton = document.querySelectorAll('.site-header__toggle-menu'),
	...
	document.querySelectorAll('.site-header__toggle-menu').forEach(function (el) {
		el.addEventListener('click', toggleMenu, { passive: true })
	});
	...
	let toggleMenuItem = function (e) {
		if ((e.target.tagName == 'A' && e.target.parentElement.classList.contains('menu-item-has-children')) || (e.target == this)) {
			...
			if( this.classList.contains('active') ) {
				...
				parent.querySelectorAll('.menu-item-has-children').forEach(el => {
					el.classList.remove('active');
					el.style.marginBottom = "0";
				});
	...
	document.querySelectorAll('.top-nav .menu-item-has-children').forEach(function (el) {
		el.addEventListener('click', toggleMenuItem, { passive: true })
	});
	/**End Mobile Menu */
	...
	/**Start choose lang */
	let langBtn = document.querySelectorAll('.langs__show');
	let showLangs = (e) => {
		e.target.parentElement.classList.toggle('langs--show');
		
	}
	langBtn.forEach(el => {
		el.addEventListener('click', showLangs, { passive: true });
	})
	/**End choose lang */

	/**Tabs start */
	let productTabNames = document.querySelectorAll('.home-catalog__tab-name'),
		productTabClose = document.querySelectorAll('.home-catalog__close');
	productTabNames.forEach( el => {
	...
	productTabClose.forEach( el => {
		el.addEventListener('click', function(){
			let tabs = document.querySelector('.home-catalog__tabs'),
				tabBtnActive = document.querySelector('.home-catalog__tab-name--active'),
				pb = "0px",
				tabNameActive,
				tabActive,
			...
			tabs.style.paddingBottom = pb;
			tabs.querySelectorAll('a').forEach(el => {
	...
	let fileInputs = document.querySelectorAll('.file input[type=file]');
	...
	allForms.forEach(form => {
		...
		form.addEventListener('change', function (e) {
			if (e.target.closest('.files__item')) {
				let item = e.target.closest('.files__item'), name = item.querySelector('.files__item-name');
	...
	let delFiles = document.querySelectorAll('.files__del');
	delFiles.forEach(del=>{
		del.addEventListener('click', function(e) {
			let curForm = e.target.closest('form'),
	...
	/**Floor plan start */
	let addApartDetails = (el) => {
		let apartDetail = document.createElement('div'),
		...
		if (elCoord['x'] < 350) {
			...
			let elLeft = elCoord['x'] + elCoord['width'];
	...
	let addModalDetails = (el) => {
		let modalDetailRoom = document.querySelector('.modal-apart__detail-item--room'),
	...
	let apart = document.querySelectorAll('.plan__floor svg .apartment');
	apart.forEach( el => {
		...
		el.addEventListener( 'mouseleave', function() {
			...
			document.querySelectorAll('.apartment__detail').forEach(el => {el.remove();});
		}, {passive:true});
	...
	if (typeof MicroModal !== 'undefined') {
		...
		let popupBtn = document.querySelectorAll('[data-src]');
		...
		document.querySelectorAll('.modal-apart__btn').forEach(el => {
		...
		document.addEventListener('wpcf7mailsent', function (event) {
			let openModal = document.querySelector('.modal.is-open');
			if( openModal ) {
	...
	if (typeof gsap !== 'undefined') {
		...
		window.addEventListener('load', function () {
			gsap.timeline({
				...
				.to(".interior__scroll img", { x: 400 }, 0);
